...
public class QuestionService {
 private final TopicRepository topics; private final QuestionRepository questions; private final AiGeneratorService generator; private final QuestionMapper mapper; private final AnswerHistoryService history; private final QuestionValidator validator;
 @Transactional
 ...
 public synchronized List<QuestionResponseDTO> getOrGenerateQuestions(String topicName,int amount){
  ...
  removeInvalidCachedQuestions(topic.getId());
 ...
 private void removeInvalidCachedQuestions(Long topicId){var invalid=questions.findByTopicIdOrderByCreatedAtAsc(topicId).stream().filter(question->!validator.isUsable(question)).toList();if(!invalid.isEmpty()){questions.deleteAll(invalid);questions.flush();}}
}
